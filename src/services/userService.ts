    ...
    try {
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError) {
            ...
            if (error.code === "P2002") {
                ...
                // .find()와 비슷한 역할이지만,
                // find는 조건을 걸어서 찾을 수 잇는 메서드이고 (리턴값은 찾은 그 요소)
                // includes는 단순히 집어넣은 값과 환벽히 같은 것이 있는지 true/false로 찾아줌
                if (errorMessage.includes("username")) {
                    // 상위 함수로 던지는데, 새로운 자바스크립트 표준 에러 객체를 만들어서 던짐.
                    // 그 내용은 "ALREADY_EXISTS_USERNAME"이라고 담아서.
...
const login = async (data: LoginInputType) => {
    ...
    // password와 deletedAt라는 항목을 response(응답)에 포함시킬 필요 없어서, 그걸 제외한 나머지만 safeUserInfo에 저장
